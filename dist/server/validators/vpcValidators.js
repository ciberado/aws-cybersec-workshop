 ...
 */
export async function validateVPCArchitecture(credentials) {
 ...
 */
export async function validateRouteTables(credentials) {
    const ec2Client = new EC2Client({
        region: 'us-east-1',
        credentials: {
            accessKeyId: credentials.aws_access_key_id,
 ...
 * Find VPC using tags
 */
async function findProjectVPC(ec2Client) {
    try {
 ...
 * Get VPC information
 */
async function getVPCInfo(ec2Client, vpcId) {
    const response = await ec2Client.send(new DescribeVpcsCommand({
        VpcIds: [vpcId]
    }));
 ...
 */
function validateCIDRDesign(vpcInfo) {
    const details = [];
 ...
 */
async function validateBasicSubnetArchitecture(ec2Client, vpcId) {
    const details = [];
 ...
 */
function validateBasicSubnetCapacity(subnets) {
    const details = [];
    let isValid = true;
 ...
 */
async function validateInternetGateway(ec2Client, vpcId) {
    const details = [];
    try {
        ...
        const isAttached = attachment?.State === 'attached' || attachment?.State === 'available';
        return {
            isValid: isAttached,
 ...
 */
async function validateNATGateway(ec2Client, vpcId) {
    const details = [];
    try {
 ...
 */
async function validateRouteTableConfiguration(ec2Client, vpcId) {
    const details = [];
    try {
        // Get route tables
 ...
 */
function classifySubnetByRoutes(subnet, routeTables) {
