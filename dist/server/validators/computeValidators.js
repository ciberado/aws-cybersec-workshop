...
import { EC2Client, DescribeSubnetsCommand, DescribeRouteTablesCommand, DescribeLaunchTemplatesCommand, DescribeLaunchTemplateVersionsCommand, DescribeImagesCommand } from '@aws-sdk/client-ec2';
import { AutoScalingClient, DescribeAutoScalingGroupsCommand, DescribeTagsCommand as ASGDescribeTagsCommand } from '@aws-sdk/client-auto-scaling';
 ...
 */
export async function validateLaunchTemplate(credentials) {
    const ec2Client = new EC2Client({
        region: 'us-east-1',
        credentials: {
            accessKeyId: credentials.aws_access_key_id,
...
async function discoverProjectLaunchTemplate(ec2Client) {
    try {
...
async function validateLaunchTemplateConfig(ec2Client, launchTemplate) {
    try {
...
async function validateTemplateAMI(ec2Client, templateData) {
    try {
        const details = [];
        let valid = true;
...
function validateUserData(templateData) {
    const details = [];
    let valid = true;
 ...
 */
export async function validateAutoScalingGroup(credentials) {
...
async function discoverProjectASG(asgClient) {
    try {
...
function validateASGConfig(asg) {
    const details = [];
    let valid = true;
    ...
    if (targetGroups.length === 0) {
        ...
        const maintgRegistered = targetGroups.some((arn) => arn.includes('maintg'));
        if (maintgRegistered) {
            ...
            details.push(`Current target groups: ${targetGroups.map((arn) => arn.split('/')[1]).join(', ')}`);
        }
    }
...
async function validateASGSubnetPlacement(ec2Client, asg) {
    try {
        const details = [];
        let valid = true;
        ...
        const subnetClassifications = new Map();
        for (const subnet of Subnets) {
            const tier = classifySubnetTier(subnet, RouteTables || []);
...
function validateASGInstances(asg) {
    const details = [];
    let valid = true;
    ...
    if (instances.length === 0) {
        ...
        const inServiceInstances = instances.filter((i) => i.LifecycleState === 'InService').length;
